...
def deposit(amount):
    ...
    if check_multiplicity(amount):
        bank_account += amount
        operations.append(
            f"Пополнение карты на {amount} у.е. Итого {decimal.Decimal(bank_account):.0f} у.е."
        )
...
def withdraw(amount):
    ...
    global operations
    if check_multiplicity(amount):
        if bank_account - amount - (amount * PERCENT_REMOVAL * 10) >= 0:
            bank_account -= amount + decimal.Decimal((amount * PERCENT_REMOVAL * 10))
            operations.append(
                f"Снятие с карты {amount} у.е. Процент за снятие {decimal.Decimal(amount*PERCENT_REMOVAL*10):.0f} у.е.. Итого {decimal.Decimal(bank_account):.0f} у.е."
            )
        else:
            operations.append(
                f"Недостаточно средств. Сумма с комиссией {amount+(amount*PERCENT_REMOVAL*10):.0f} у.е. На карте {decimal.Decimal(bank_account):.0f} у.е."
            )
...
def exit():
    ...
    global operations
    global RICHNESS_SUM
    global RICHNESS_PERCENT
    if bank_account >= RICHNESS_SUM:
        bank_account -= bank_account * RICHNESS_PERCENT
        operations.append(
            f"Вычтен налог на богатство {RICHNESS_PERCENT}% в сумме {decimal.Decimal(bank_account*RICHNESS_PERCENT):.0f} у.е. Итого {decimal.Decimal(bank_account):.0f} у.е."
        )
    operations.append(
        f"Возьмите карту на которой {decimal.Decimal(bank_account):.0f} у.е."
    )
    pass
